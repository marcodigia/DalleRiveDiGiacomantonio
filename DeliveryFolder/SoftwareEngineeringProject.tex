...
\begin{itemize}
	...
	\item \textbf{Threshold:} Flexible value related to the biomedical data acquired by the wearable device in which the system-to-be is installed. This value is computed by well-known equation that operate with user's data. It also depends of the kind of activity that a user is doing.  
...
\begin{itemize}
%	\item {[R1]} \boldtf (if user’s heartbeat overcome the threshold the system must call an ambulance within 5 seconds (...) THE DOCUMENT SAYS BELOW —> ASK! ).
	\item Ambulance Service
\end{itemize}
